...
func to_btree() {
    proc_sql := "select count(*) from information_schema.processlist where user not in ('replicater','root','system user','backup','monitor')"
    for _, value := range results {
        var do_str string = "  "
        if len(strings.Split(value, ":")) == 1 {
            fmt.Println("Master:" + value + "          Connection:  " + get_data(value, proc_sql)[0])
        } else if len(strings.Split(value, ":")) > 1 {
            for t_len := 0; t_len < len(strings.Split(value, ":")); t_len++ {
                do_str += do_str
            }
            fmt.Println("Level" + strconv.Itoa(len(strings.Split(value, ":"))) + ":" + do_str + strings.Split(value, ":")[len(strings.Split(value, ":"))-1] + "          Connection:  " + get_data(strings.Split(value, ":")[len(strings.Split(value, ":"))-1], proc_sql)[0])
        }
    }
}
func main() {
