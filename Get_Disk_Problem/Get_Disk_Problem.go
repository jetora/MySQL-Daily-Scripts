...
func get_slave(ipstr string) []string {
	...
	if len(strings.Split(ipstr, ":")) == 1 {
		slave_ip_arr = get_data_arr(ipstr, slave_ip_sql)
	} else {
		slave_ip_arr = get_data_arr(strings.Split(ipstr, ":")[len(strings.Split(ipstr, ":"))-1], slave_ip_sql)
	}
...
func to_btree() {
    const(
        binlogstr= "binlog count"
        backupstr= "backup size"
    )
    for _, value := range results {
        var do_str string = "  "
        if len(strings.Split(value, ":")) == 1 {
            fmt.Println("Master:" + value)
            ssh.Ssh(binlogstr,value,"/bin/ls -l /export/data/mysql/data/mysql-bin.*|wc -l")
            ssh.Ssh(backupstr,value,"/usr/bin/du -sh /export/data/mysql/dumps/|awk -F ' ' '{print $1}'")
            ...
            ssh.Ssh(binlogstr,strings.Split(value, ":")[len(strings.Split(value, ":"))-1],"/bin/ls -l /export/data/mysql/data/mysql-bin.*|wc -l")
            ssh.Ssh(backupstr,strings.Split(value, ":")[len(strings.Split(value, ":"))-1],"/usr/bin/du -sh /export/data/mysql/dumps/|awk -F ' ' '{print $1}'")
        }
    }
}
func main() {
